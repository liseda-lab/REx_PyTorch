...
- A PyTorch implementation of REx (IJCAI [paper](https://www.ijcai.org/proceedings/2025/0515.pdf)). The original TensorFlow implementation lives at [liseda-lab/REx](https://github.com/liseda-lab/REx).
- The persona-conditioned framework from a new submission. The reference implementation and reproducibility artifact for that work are at [liseda-lab/Perspective_XAI](https://github.com/liseda-lab/Perspective_XAI); this PyTorch port adds engineering improvements (decoupled rerank, local-LLM support) and extends the approach to **Oregano DR/DTI** and **PrimeKG** beyond the Hetionet datasets used in the paper.
- Original (non-personalized) REx is reproduced via the `neutral_evaluator` configs.
...
Where `{dataset}` is the name of the dataset you would like to run the approach on, and `{task}` is `drug_repurposing` or `drug_target` (interaction). `{persona}` selects the persona-conditioned variant; for the original implementation of REx always choose `neutral_evaluator`. Take this example:
...
### Datasets
Bundled datasets (under `datasets/`): `hetionet/{drug_repurposing,drug_target}/`, `oregano/{drug_repurposing,drug_target}/`, `primekg/drug_repurposing/`.

Each dataset directory has the following files:
```
dataset
    ├── graph.txt
    ├── dev.txt
...
Where:
- `graph.txt` contains all triples of the KG except for `dev.txt` and `test.txt`.
...
- `clustered_IC_classes_edgeType.json` contains the IC scores for each edge type of the graph. It is a dictionary where the keys are edge types and the values are dictionaries with the IC scores for each class.
...
**Note 1**: The existing datasets have the `graph.txt` file divided into smaller parts (`graph_part*.txt`) due to GitHub file size limits. The system **automatically assembles** `graph.txt` from these parts on the first run, meaning no manual steps are needed.

**Note 2**: The persona-conditioned version of REx uses a large language model for persona-shaped scoring. There are three modes controlled by `--llm_api` and `--llm_model`:

| Mode | Flag | Model | Requirement |
|------|------|-------|-------------|
| Local (default) | `--llm_api 0` | Set by `--local_model` | High-performance GPU (~32 GB of VRAM) necessary |
...
For local mode, the `--local_model` parameter controls which model is loaded (default: `Qwen/Qwen3.5-9B`). The model weights are automatically downloaded from HuggingFace and cached in `~/.cache/huggingface/hub/`. The download can take several minutes depending on model size and connection speed:
...
The scorer can also run standalone against any saved paths.json:

```sh
uv run python code/model/score_external.py <paths.json> <persona.txt> [threshold] [alpha] [test_data.txt]
```
